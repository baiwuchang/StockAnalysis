...
class Calendar:
    ...
    def __place_widgets(s):
        ...
        s.CB_year = ttk.Combobox(hframe, width = 5, values = [str(year) for year in range(datetime.now().year, datetime.now().year-6,-1)], validate = 'key', validatecommand = (Input_judgment_num, '%P'))
        s.CB_year.current(0)
...
if __name__ == '__main__':
    ...
    date_str_gain = lambda: [
        ...
        if date]
    tk.Button(root, text = '选择日期:', command = date_str_gain).place(x = 0, y = 0, relx = 1/20, rely = 1/6, relwidth = 4/20, relheigh = 2/3)
    root.mainloop()
